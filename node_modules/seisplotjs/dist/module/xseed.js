...
class XSeedHeader {
  ...
  static createFromDataView(dataView) {
    ...
    header.identifier = makeString(dataView, 40, header.identifierLength); // lazily extract json and data
  ...
  timeOfSample(i) {
    return moment$1.utc(this.start).add(1000 * i / this.sampleRate, 'milliseconds');
  }
  /**
  ...
  _startToMoment() {
    let m = new moment$1.utc([this.year, 0, 1, this.hour, this.minute, this.second, 0]);
...
function areContiguous(dr1, dr2, sampRatio = 1.5) {
  let h1 = dr1.header;
  let h2 = dr2.header;
  return h1.end.isBefore(h2.start) && moment$1.utc(h1.end).add(sampRatio / h1.sampleRate, 'seconds').isSameOrAfter(h2.start);
}
...
export { CRC_OFFSET, FDSN_PREFIX, FIXED_HEADER_SIZE, LITTLE_ENDIAN, UNKNOWN_DATA_VERSION, XSeedHeader, XSeedRecord, areContiguous, byChannel, calculateCRC32C, convertMS2Record, convertMS2toXSeed, crcToHexString, createSeismogramSegment, makeString, merge, padZeros, parseExtraHeaders, parseXSeedRecords, seismogramPerChannel };
//# sourceMappingURL=xseed.js.map
