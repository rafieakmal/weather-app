...
function log(msg) {
  ...
  select("div#debug").append("p").text("".concat(stringify(msg)));
}
/**
...
function calcClockOffset(serverTimeUTC) {
  return moment$1.utc().diff(serverTimeUTC, 'seconds', true);
}
/**
...
class StartEndDuration {
  constructor(startTime, endTime, duration = null, clockOffset = 0) {
    if (isDef(duration)) {
      if (typeof duration === "string" || duration instanceof String) {
        if (duration.charAt(0) === 'P') {
          this._duration = moment$1.duration(duration);
        } else {
          this._duration = moment$1.duration(Number.parseFloat(duration), 'seconds');
        }
      } else if (typeof duration === "number" || duration instanceof Number) {
        this._duration = moment$1.duration(duration, 'seconds');
      } else if (moment$1.isDuration(duration)) {
        this._duration = moment$1.duration(duration);
      } else {
    ...
    if (startTime && endTime) {
      ...
      this._duration = moment$1.duration(this.endTime.diff(this.startTime));
    } else if (startTime && this._duration) {
      this._startTime = checkStringOrDate(startTime);
      this._endTime = moment$1.utc(this.startTime).add(this.duration);
    } else if (endTime && this._duration) {
      this._endTime = checkStringOrDate(endTime);
      this._startTime = moment$1.utc(this.endTime).subtract(this.duration);
    } else if (this._duration) {
      if (clockOffset === undefined) {
        this._clockOffset = moment$1.duration(0, 'seconds');
      } else if (clockOffset instanceof Number) {
        this._clockOffset = moment$1.duration(clockOffset, 'seconds');
      } else {
        this._clockOffset = clockOffset;
      }

      this._endTime = moment$1.utc().subtract(clockOffset);
      this._startTime = moment$1.utc(this.endTime).subtract(this.duration);
    } else {
...
function checkStringOrDate(d) {
  if (moment$1.isMoment(d)) {
    return d;
  } else if (d instanceof Date) {
    return moment$1.utc(d);
...
export { JSONAPI_MIME, JSON_MIME, StartEndDuration, TEXT_MIME, XML_MIME, calcClockOffset, checkProtocol, checkStringOrDate, dataViewToString, defaultFetchInitObj, doFetchWithTimeout, hasArgs, hasNoArgs, isDef, isNonEmptyStringArg, isNumArg, isObject, isStringArg, log, makeParam, meanOfSlice, stringify, toIsoWoZ };
